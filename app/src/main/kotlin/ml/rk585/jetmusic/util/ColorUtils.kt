    ...
    LaunchedEffect(image, fallback, isDarkColors) {
        if (image != null && imageSource != null) {
            accent = adaptiveColorCache.getOrPut(imageHash) {
                ...
                getAccentColor(isDarkColors, fallback.toArgb(), palette).run {
                    Color(this)
                }
            }
...
private fun getAccentColor(isDark: Boolean, default: Int, palette: Palette): Int {
    when (isDark) {
        true -> {
...
private fun Color.contentColor(): Color {
    return getContrastColor(toArgb()).run { Color(this) }
}
